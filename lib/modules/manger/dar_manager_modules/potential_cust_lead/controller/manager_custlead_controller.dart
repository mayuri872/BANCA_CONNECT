...
class ManagerCustLeadController extends GetxController {
  ...
  RxList<ResponseManagerDashModel> responceAgentLeadDetails =
      RxList<ResponseManagerDashModel>();
       ...
       RxList<ResponseDailyActivityReport> custLead =
  ...
  Future<void> getManagerDashData(Map<String, dynamic> item) async {
    ...
    if (flag > 1) {
      ...
      Get.toNamed(AppRoutes.managerCustLeadActivityReport);
    }
  }

  Future<void> getDSRCustomerLeadsdtls(empCode) async {
    if (empCode != null) {
      await getManagerDashDataServices.getDSRCustomerLeadsdtls(
        userName: empCode,
        onSuccess: (res) {
          custLead.value = res;
          isDataLoaded(true);
